    ...
    *out_idx = -1;
    ...
    while (top > 0) {
        ...
        auto aabb_sqr = [&](int ci) -> double {
            ...
            for (int k = 0; k < 3; ++k) {
                if (px[k] < ch.aabb_min[k]) {
                    double diff = ch.aabb_min[k] - px[k];
                    d += diff * diff;
                } else if (px[k] > ch.aabb_max[k]) {
                    double diff = px[k] - ch.aabb_max[k];
                    d += diff * diff;
                }
            }
            return d;
...
void gpu_plain_free_B() {
    ...
    cudaFree(g_d_tris_B);
    g_d_tris_B = nullptr;
    cudaFreeHost(g_h_pts);
    g_h_pts = nullptr;
    cudaFreeHost(g_h_nearest);
    g_h_nearest = nullptr;
    cudaFree(g_d_pts);
    g_d_pts = nullptr;
    cudaFree(g_d_nearest);
    g_d_nearest = nullptr;
